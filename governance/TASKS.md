...
|---|---|---|---|---|---|
| DMR-010 | `in_progress` | HUB → DMR open-issue sync: mirror the 8 still-open HUB cards (HUB-044/064/054/062/042/020/061/036) onto the DMR board with synced issues + cross-links; HUB-006 reviewed and skipped (no equivalent surface in this codebase). | opencode (deepseek-v4-flash-vision-exp) 2026-09-09 | — | — |
| DMR-011 | `assigned` | Harden `sync_packages.py patch_push` so uncommitted work can never propagate: copy committed blobs (`git archive HEAD` / `git ls-tree`) and/or re-assert a per-package clean-tree guard before each package copy; retire the AGENTS.md + wiki race caveat when done. | unclaimed | — | synced from HUB-044 [#28](https://github.com/Exios66/mailroom-dev/issues/28); race still present at `scripts/sync_packages.py:345-351` |
| DMR-012 | `assigned` | Gmail triage production-readiness residuals: live re-send/provenance operator proof, CI secret-presence check (`GMAIL_ADDRESS`/`GMAIL_APP_PASSWORD`), Langfuse/Phoenix trace checklist for `gmail-triage`, ops-lamp alerting on `checks.gmail_intake`, and watcher-stability/free-swarm/failed-bin runbooks. HUB-037/039/043/048 deliveries are in place — do not redo. | unclaimed | — | synced from HUB-064 [#43](https://github.com/Exios66/mailroom-dev/issues/43) |
| DMR-013 | `assigned` | Run the live `packages/The-Mailroom/scripts/publish_pages.sh` so the terminal site goes live at the Pages root (currently the v0.3.0 pixel console is served; `/terminal/` 404s) and verify `publish_pages.sh --status`; TUI/site code, 0.4.0 release, and upstream sync are already delivered. | unclaimed | — | synced from HUB-054 [#25](https://github.com/Exios66/mailroom-dev/issues/25); only the live publish is pending |
| DMR-014 | `needs_attention` | Specialist mutations on mailroom-corpus v7+v8: implement structured specialist mutation operators + fitness/registry and run the first evolution pass. `needs:` tokenizer-budget alignment (GT-integrity half now evidenced by the HUB-032 reconciliation + `content_sha256` loader proof). | unclaimed | — | synced from HUB-062 [#11](https://github.com/Exios66/mailroom-dev/issues/11); no mutation operators/registry exist yet |
| DMR-015 | `assigned` | Derive mailroom-named successor keys for the frozen pre-v8 insurance-subclass surfaces — vision sorter rule 40 (`sorter_docclass_vision_v1`), `_PILOT_CONTEXT`, `reviewer_docclass_pilot_v0` — teaching the v8 six-token set (carrier/pde/outpatient/inpatient + property/auto), plus the docclass-pilot examples refresh; same-surface A/Bs before any default change. Coordinate `_PILOT_CONTEXT` with DMR-016. | unclaimed | — | synced from HUB-042 [#30](https://github.com/Exios66/mailroom-dev/issues/30); surfaces still 4-token at `src/prompts.py:1266`, `src/prompts_docclass.py:702-703,752-755` |
| DMR-016 | `needs_attention` | Add NEW docclass judge/reviewer/arbiter/boss prompt keys with five-class + `unknown` grading language per plan §67, mirror byte-identical into The-Mailroom (and re-sync the stale 32→59-key mirror), defaults unchanged until a same-surface A/B. `decision:` keep the extended 8-class surface as deliberate eval design (KANBAN-033 lineage) or move to five-class grading. | unclaimed | — | synced from HUB-020 [#26](https://github.com/Exios66/mailroom-dev/issues/26); extended-8 grading persists at `src/prompts_docclass.py:92-110,246-250,313-315,371-372` |
| DMR-017 | `needs_attention` | GEPA prompt mutations on docclass v7+v8: run the prompt-engineer/GEPA loop against the five-class corpus surface and register the first `mailroom_v` docclass lineage keys (sorter + judge/reviewer) with same-surface A/Bs. `needs:` the DMR-016 five-class decision; never mutate frozen `docclass_*` keys. | unclaimed | — | synced from HUB-061 [#7](https://github.com/Exios66/mailroom-dev/issues/7); only `sorter_mailroom_v0` (HUB-041) exists |
| DMR-018 | `assigned` | v9 corpus candidate: re-evaluate INSURBIAS (CC-BY-4.0 insurance-claim narratives) as a v9 source alongside the P4 expansion priorities — insurance workflow is now MEDIUM with adjuster 150/950 after v8 — license permitting and only with decision/adjudication GT or an explicit GT-gap convention. | unclaimed | — | synced from HUB-036 [#27](https://github.com/Exios66/mailroom-dev/issues/27); v9 not started, INSURBIAS deferred at `packages/mailroom-corpus-eda/AGENTS.md:131` |
