...
Pre-warm the HF cache volume before the first real boot (skips the cold-boot
weight download):

    modal run deploy/modal_vllm.py::download_model

Verify a deployed endpoint (bearer-aware, loud diagnostics):
...
model, GPU size, or quantization:
    ...
    MODAL_VLLM_MAX_MODEL_LEN   int tokens            (default 16384; 32768 only for AWQ/FP8 rows — vLLM v0.28.0 RAISES at boot on L4-bf16 8B-class rows at 32768, DMR-056)
...
# DMR-056: cost-allocation tags — parity with the sandbox/llm-mailroom sibling apps.
app = modal.App(
    APP_NAME,
    image=image,
    tags={
        "project": "digital-mailroom",
        "package": "llm-entity-extraction",
...
def serve() -> None:
    ...
    print("=== entity-vllm serve config (masked) ===")
...
def _smoke_check(base: str) -> None:
    ...
    if not base:
        raise SystemExit(
            ...
            "(https://<workspace>--entity-vllm-serve.modal.run/v1)"
        )
