...
Timeouts, 401/403, 429s, and I/O failures must not look like soft review
escalations in the catalog / floor trays (llm-mailroom v0.6.0).
"""
...
_BUDGET_MARKERS = ("run budget", "deadline exceeded", "token budget", "max turns")
...
def classify_run_failure(exc: BaseException) -> dict[str, str]:
    ...
    if any(m in low for m in _BUDGET_MARKERS):
        return {"failure_class": RUN_BUDGET, "reason": f"{name}: {detail}", "detail": detail}
...
def aborted_escalation(classified: dict[str, str]) -> str:
    fc = classified.get("failure_class") or UNEXPECTED
    reason = classified.get("reason") or "unexpected error"
    return f"run aborted [{fc}]: {reason}"
