...
|---|---|---|---|---|---|
| DMR-004 | `in_progress` | **Push-triggered deploys for the served board — Vercel Git integration** — human directive 2026-09-09: install the Vercel GitHub App on `LLM-Mailroom-Services` (done) and connect the `digital-mailroom` project to this repo. Scope: `vercel git connect` (linked to `LLM-Mailroom-Services/Digital-Mailroom`, production branch `main`, Root Directory `board-site`), then verify a push to `main` produces a Ready git-integration deployment served under the production alias. Fallback (not needed unless integration regresses): GitHub Actions with a `VERCEL_TOKEN` secret. Spawned from DMR-002 close. | opencode (deepseek-v4-flash-vision-exp) | [#8](https://github.com/LLM-Mailroom-Services/Digital-Mailroom/issues/8) | opened 2026-09-09 by human directive; Vercel GitHub App installed on the org (human, 2026-09-09); `vercel git connect` succeeded — project linked (`repo: Digital-Mailroom`, `productionBranch: main`, `rootDirectory: board-site`); push-triggered deploy verification in flight. |
