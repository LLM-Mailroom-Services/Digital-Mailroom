...
| DMR-007 | `in_progress` | **Retire stray `board-site` Vercel project + served-board write-path proof** — the duplicate Vercel project `board-site` (id `prj_8zQcxU0FxqXPW4qmE8reQQuXFciu`, URL board-site-gamma.vercel.app → 404, linked to `Exios66/mailroom-dev` with Root Directory unset) is retired so the only served board is the canonical `digital-mailroom` project (digital-mailroom-theta.vercel.app). Write-path proof for the human directive "new DMR issues tracked in the org repository and displayed on the new Vercel deployment": this card was created through `POST /api/board` on the served site, landed as a kanban issue in `LLM-Mailroom-Services/Digital-Mailroom` (#11), and displays live; lane corrected via `PATCH /api/board/DMR-007`. | opencode (deepseek-v4-flash-vision-exp) | [#11](https://github.com/LLM-Mailroom-Services/Digital-Mailroom/issues/11) | created 2026-09-09 via the served board (POST → org issue #11); PATCH lane fix verified; stray project deleted via the Vercel API. |
