...
| DMR-043 | `unassigned` | Sandbox docker-compose — fix jupyter build context/volume resolution (`context: .` -> `..`, mount `- ..:/workspace`, add `name:`) + vllm GPU reservation (missing nvidia device reservation). | unclaimed | [#46](https://github.com/LLM-Mailroom-Services/Digital-Mailroom/issues/46) | docker-deployment-specialist 2026-09-10: verified via `docker compose config` |
...
| DMR-045 | `unassigned` | Expand sandbox vLLM model catalog (`config/models.yaml`) with verified per-GPU-class rows (L4/A10G/A100/H100, quantization, max_model_len) + add `MODAL_VLLM_TP_SIZE` knob -> `--tensor-parallel-size` for 70B-class. | unclaimed | [#48](https://github.com/LLM-Mailroom-Services/Digital-Mailroom/issues/48) | vllm-specialist + modal-specialist 2026-09-10: model tables + 14B-trap + TP2 gap verified |
