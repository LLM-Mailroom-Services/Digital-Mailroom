...
| DMR-041 | `unassigned` | Sandbox job CLI — wire whole-run eval task delegation (pipeline/extract/chained/local_vs_api/isolated): `RUNNABLE_TASKS` advertises them but `_predict_row` raises ValueError; `run_job` must delegate to the existing whole-run eval runners. | unclaimed | [#44](https://github.com/LLM-Mailroom-Services/Digital-Mailroom/issues/44) | Audit 2026-09-10: runner.py:22-23 vs :26-38; reproduced ValueError for pipeline |
| DMR-042 | `unassigned` | Sandbox corpus Hub loader — fix `load_hf` NameError (corpus.py:228) + `effective_revision` AttributeError (corpus.py:73,234 / preflight.py:124); HF corpus path is completely broken for non-file specs. | unclaimed | [#45](https://github.com/LLM-Mailroom-Services/Digital-Mailroom/issues/45) | lucius audit 2026-09-10: reproduced live; only file:// paths tested |
