...
| DMR-044 | `unassigned` | CHTC batch eval — install eval agents (`llm-entity-extraction@v0.20.0 --no-deps` + `openai>=1.30`) so runs are live-or-loud, parity flags (`MAX_MODEL_LEN:-32768`/`GPU_MEMORY_UTILIZATION:-0.90`/`MAX_NUM_SEQS:-256`), post-eval offline_fallback guard. | unclaimed | [#47](https://github.com/LLM-Mailroom-Services/Digital-Mailroom/issues/47) | docker-deployment-specialist 2026-09-10: silent-mock defect at run_batch_eval.sh:41,44,72-74 |
