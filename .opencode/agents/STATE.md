...
- **Date:** 2026-09-22 (UTC)
- **Scope:** DMR-076 / issue #77 — operator-desk Docker production path (commits 0fffd724 + 5be2a56c + b215ff23 on `cost-compare/L4-vs-openrouter`)
- **Claim vs. Evidence verdict:** EVIDENCE-GROUNDED on all 7 claims. `git show --stat 5be2a56c` = exactly the 17 claimed files. Spot-checks all pass: Dockerfile has `AS ui-builder` (:32), `AS operator` (:92) with `/data` pre-created mailroom-owned (:111) and trailing `FROM runtime AS observatory` (:117); compose has 3 services only (mailroom/mailroom-observer/nginx), `:?` fail-fast on both secrets with no `:-` default (:26-27), `target: operator` + `MAILROOM_EXTRAS` arg on both app services, nginx `80:80` the only publish, `service_healthy` deps; nginx.conf `location /` proxies with `try_files` removed (documented why, :58-64), /api/ /v1/ /ws /ws/pipeline kept; ui/Dockerfile standalone-repaired with `VITE_BASE=/` (:13), ui/nginx.conf has legit try_files in a non-proxy root-serving location (:43, correct — it has a local root), new ui/.dockerignore; all 9 doc surfaces carry operator content; test_operator_compose.py (84 lines, pins the whole shape) + test_publish_space.py stage assertions present. Gates re-run by me: `pytest test_operator_compose.py + test_publish_space.py` → 11 passed; full suite → 316 passed / 4 failed + test_tui collection error — the 4 failures are exactly the card's declared pre-existing Windows issues (cp1252 README decode, poller session overlay, health revalidation, spa-contract em-dash) in files the card never touched; `docker compose config` unset → exit 1 naming both vars, set → only published port 80. Branch pushed: `git ls-remote origin cost-compare/L4-vs-openrouter` = b215ff23 = local HEAD.
- **Actions taken:** None required — no gaps. Card honestly records its own deferrals (Exios66 mirror push 403-blocked, issue #77 label-sync deferred, no GITHUB_TOKEN). Housekeeping note (not a card defect): uncommitted local `M AGENTS.md` (+43 lines Devin roster mirror) + untracked `.devin/` present in worktree — outside card scope but should be committed or stashed before the next release-train sweep.

## Previous Audit

- **Date:** 2026-09-16 (UTC)
- **Scope:** DMR-064 sub-unit — compliance-specialist removal verification (READ-ONLY; commits 59c47401 / be3d2973 / 47a03acd / bbee4bf5)
- **Claim vs. Evidence verdict:** PARTIALLY CONFIRMED. llm-mailroom live tree = fully removed (8 D + 48 M in 59c47401; zero tokens in src/; taxonomy.yaml = 5 classes + unknown). Monorepo-wide = NOT fully removed: LIVE in local-mailroom-sandbox (components.yaml:27 true; eval/agents.py:32,40; prompt_registry.py:33; tracked vendor snapshot v0.7.1 @ 2a212e76 contains the full specialist+docclass arm), llm-dojo-scoring (LIVE_DOC_CLASS_KEYS:163/168, LIVE_SPECIALIST_AGENTS:78, catalog/templates/suites), llm-entity-extraction (own ComplianceFilingSpecialist, deliberate per DMR-046), agent-mailroom (own taxonomy + desk-compliance). STALE: llm-mailroom docs/agents.md:154-164 (deleted file), docs/local-models.md:107-108/173 (cutover cmds), docs/testing.md:21 (fixture tree), README.md:253 ("status: retired" claim), root docs/v7-taxonomy.md:113 + DOCCLASS_CONTRACT.md:40-43 (claim retained entry — removed), .opencode/agents/prompt-engineer.md:119/135 (deleted file), The-Mailroom docclass_prompts.py mirror + UI fallbacks, llm-mailroom-graph generated artifacts.
- **Actions taken:** doc-drift fixes shipped in the DMR-064 cementing commit (agents.md/local-models.md/testing.md now token-free; v7-taxonomy.md + DOCCLASS_CONTRACT.md updated to removal state; prompt-engineer.md mailroom half dropped; The-Mailroom mirror frozen with dated comment); guard test `scripts/tests/test_compliance_removal_guard.py` added (fails loudly on any re-import of the 5 cursor-gap paths / tokens / taxonomy drift). Verified merge base aace6a49 / upstream d62a169f blobs → 4/5 gap paths resolve clean as deletions, skills/compliance_specialist/README.md is add-delete (needs --resolve ours), 29 both-touched paths need ours; comm proof that no compliance-content upstream file sits outside the removal set.
- **Follow-up (not this card):** sibling packages carry the specialist by design or by stale roster (sandbox vendor refresh = DMR-057; dojo LIVE rosters factually wrong vs post-removal five — needs its own card; entity = DMR-046 pending; agent-mailroom "Six live doc classes" README claim = its own standing surface); llm-mailroom-graph artifacts self-heal on rebuild. Pre-existing non-defect: taxonomy_parity.py voicemail correspondence-subclass drift (dojo corpus.py + entity sorter_agent.py) — unrelated to the docclass arm, gate was red before this card.
...
- **Actions taken:** Fixed docker-compose.yml (context/mount/name/GPU/port/user), .dockerignore, Dockerfile comments, modal_vllm.py (TP knob + Secret), config/models.yaml (13-model matrix), deploy/README.md (knob + model guidance). Added 7 network-free tests. Closed issues #46/#48. Archived both cards. Suite 151 passed / 1 skipped.
...
| ------ | ------- | ---------- | --------- | ---------- |
| 2026-09-22 | DMR-076 / issue #77 operator-desk Docker production path verification (read-only) | All 7 claims EVIDENCE-GROUNDED: 17-file commit 5be2a56c matches stat; Dockerfile stages + trailing observatory default; compose single-front-door + fail-fast secrets verified live via `compose config` ±secrets; nginx try_files removed from proxy only; ui standalone repair + .dockerignore; 9 doc surfaces updated; new pin tests present and green (11/11); full suite 316p/4f matches card exactly, all 4 pre-existing Windows failures in untouched files; branch pushed (b215ff23 = remote = HEAD) | STATE.md append only — no defects, no new cards | `docker compose config` exit-1-on-unset is the clean daemon-free proof of `${VAR:?}` fail-fast; ui/nginx.conf legitimately KEEPS try_files — it has a real local root, only the proxy conf must drop it |
...
| 2026-09-10 | DMR-043 + DMR-045 implementation | docker-compose jupyter context/volume resolved to deploy/ (broken build + shadowed /workspace); vllm had no GPU reservation; model catalog thin; no TP knob for 70B | Shipped all fixes (commit `73aa807f`) + 7 network-free tests; closed #46/#48; archived both cards; board 8 open, check 0/0 | YAML `notes:` values containing a bare `:` (e.g. "Reasoning model: parse") break models.yaml parsing — quote note values; `docker compose config` is the daemon-free truth for path/GPU resolution |
