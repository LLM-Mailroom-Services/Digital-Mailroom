...
def test_sorter_docclass_prompt_option_list_matches_schema():
    ...
    for version in ("sorter_docclass_v0", "sorter_docclass_v1",
        ...
        if version in ("sorter_mailroom_v0",
            ...
            # keys + the `other` fallback only). The v8-era `voicemail` key
            # left the correspondence enum at the mailroom-dataset v9 re-pin
            # (DMR-071: not in the v9 GT vocabulary), and the frozen lineage
            # never taught it.
