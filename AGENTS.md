...
| 12 | **State management** | `STATE` (project, `.opencode/agents/STATE.md`) | Maintaining STATE.md audit trail — NOT a callable subagent; read-only reference for audit history and lessons learned | *(Not invoked via Task tool — this is the board-evidence-auditor's STATE.md file. Read it before starting any audit to avoid repeating past mistakes.)* |
