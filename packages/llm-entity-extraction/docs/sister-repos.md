...
| [llm-dojo-scoring](https://github.com/Exios66/llm-dojo-scoring) | Deterministic, field-type-aware scoring engine (metric registry, T0–T3 tiers, agent profiles, doc-type bundles) | **Upstream governed dependency**, pinned in `pyproject.toml` (`.15.0`); consumed through six thin re-export shims in `src/` |
...
`mailroom-dataset`, `enron-correspondence`, `enron-correspondence-dedup`.
