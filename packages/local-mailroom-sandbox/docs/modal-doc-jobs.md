...
## 6. Decision documentation (human lane — no adjudication here)
...
1. **No GPU in the new app** — the heavy lifting lives in the deployed
   `sandbox-vllm` (L4); the queue app stays CPU-only and cheap. A GPU vision
   stage only earns its cost if scanned-document recall proves insufficient
   in Phase B.
2. **Full graph** — the doc queue is the pipeline's async front door, not a
   reduced eval profile; the judge + human-review lane are the reason the
   matter record is trusted. The reduced path remains available for
   throughput-scaled batch runs (Phase C tuning knob).
3. **Own command family** — `sandbox doc-jobs` keeps job-queue semantics
   (submit/status/pull-back) out of the run orchestrator's config surface.

Blockers acknowledged: none technical. All three are product-scope choices.
