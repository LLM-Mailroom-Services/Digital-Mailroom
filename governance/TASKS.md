...
|---|---|---|---|---|---|
| DMR-019 | `in_progress` | Adopt the `unassigned` lane on the canonical board: add it to TASKS.md/tooling/labels/docs, move every Owner-`unclaimed` card to it (free to claim by the next agent or team), teach `sync-issues` to swap stale stage labels, and verify the served board's Unassigned column. | opencode (deepseek-v4-flash-vision-exp) 2026-09-09 | [#24](https://github.com/LLM-Mailroom-Services/Digital-Mailroom/issues/24) | — |
