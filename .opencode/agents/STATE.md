...
## Latest Audit
- **Date:** 2026-09-10
- **Scope:** Sandbox Modal+vLLM job CLI + eval-task coverage + model catalog (vllm-specialist + modal-specialist); HuggingFace integration audit (lucius); Docker/container deploy surface audit (docker-deployment-specialist); Vercel board sync verification
- **Claim vs. Evidence verdict:** The `sandbox run` CLI works for preflight + sorter/legalbench per-item jobs, but 5 advertised whole-run tasks (pipeline/extract/chained/local_vs_api/isolated) crash with ValueError. HF corpus path is completely broken (undefined `load_hf`). Docker jupyter build + vllm GPU reservation broken. Model catalog needs per-GPU guidance + a TP knob.
- **Actions taken:** Opened DMR-041..DMR-045 with synced issues #44-#48; board at 12 open cards, check 0/0; updated STATE.md with full audit log + lessons. Verified Vercel board reflects issue state (38 cards, 7 open, DMR-040 closed).
