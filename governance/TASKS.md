...
|---|---|---|---|---|---|
| DMR-004 | `needs_attention` | **Push-triggered deploys for the served board — pick the mechanism** — `decision:` manual CLI deploys work today (`vercel --prod --cwd .`; repo-root `.vercelignore` ships `board-site/` only). Choose: **(a) Vercel Git integration** — install the Vercel GitHub App on the `LLM-Mailroom-Services` org (browser action, org owner), then `vercel git connect`; no secret, standard auto-deploy (Root Directory `board-site` already set); or **(b) GitHub Actions** — workflow using a `VERCEL_TOKEN` repo secret, token created at https://vercel.com/account/tokens (the CLI cannot mint tokens: 403). Spawned from DMR-002 close. | human | [#8](https://github.com/LLM-Mailroom-Services/Digital-Mailroom/issues/8) | decision needed — spawned at DMR-002 close 2026-09-09 (append-only law) |
