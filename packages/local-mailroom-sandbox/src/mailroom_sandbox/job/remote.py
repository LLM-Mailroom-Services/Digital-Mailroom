"""Remote Modal job execution for sandbox runs (DMR-027).
...
The ``modal`` module is imported lazily and ``modal volume put`` is shelled
out to, so the package's default runtime stays lean; tests stub both.
"""
...
def fire(store: RunStore, *, payload: dict | None = None) -> dict[str, Any]:
    """Push the run dir and spawn the remote job; persist the call id."""
    ...
    call = fn.spawn(payload or {"run_id": store.run_id})
