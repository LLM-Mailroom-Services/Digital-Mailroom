...
| `gmail_smoke_test.py` | HUB-037: Gmail + watcher connectivity smoke using an example insurance claim (committed FNOL fixture) as the email attachment. Default mode is network-free + mock-LLM (fake IMAP, scratch bins) and proves connectivity → inbox route → watcher claim → manifest `intake.source=gmail` → `insurance_claim` classification → `✅` reaction on the source email → completion-report echo reply on the same thread; `--real` SENDS the smoke email to the agent mailbox via SMTP and sweeps the real mailbox (drains ALL unseen messages — run on a quiet mailbox); `--llm real` runs the pipeline leg on the real LLM. |
